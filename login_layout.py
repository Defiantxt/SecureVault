...
def unlock():
    ...
    elif logic.verify_master_password_hash(password_ent.get()):
        window.destroy()
...
def sign_up():
    if len(password_ent.get()) == 0:
        ...
        window.focus()
        password_ent.configure(show="•")
    elif len(password_ent.get()) < 8:
        password_ent.delete(0, "end")
        password_ent.configure(placeholder_text="Password is too short", show="", text_color="#DCDEE1")
        window.focus()
        password_ent.configure(show="•")
    else:
        logic.setup_master_password(password_ent.get())
        window.destroy()
...
if logic.user_exists():
    ...
    unlock_vault = tk.Label(password_frm, text="Unlock Your Vault", bg="#08121F", font=("Arial", 14, "bold"),
                            fg="#DCDEE1")
    ...
    password_frm.pack(pady=(15, 15))
    password_frm.pack_propagate(False)
    create_master = tk.Label(password_frm, text="Create Master Password", bg="#08121F", font=("Arial", 14, "bold"),
                             fg="#DCDEE1")
    create_master.pack(pady=(30, 0))
    enter_pass = tk.Label(
        password_frm, text="Enter a master password to continue",
        font=("Arial", 10),
        bg="#08121F",
        fg="#DCDEE1"
    )
